...
public:
	...
	FileResult ExtractAll(utl::File& file) override
	{
		...
		for (auto& tre : fileList) {
			uint32_t offset = tre.offset & 0xFFFFFF00;
			bool isFile = tre.offset & 0xFF;
			...
			utl::File writer("unp//" + pfn + "//" +  std::to_string(isFile) + "_" + std::to_string(tre.nameHash), utl::fileMode::write);
