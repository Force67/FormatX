...
void vkRend::initResources() {
    ...
    auto alignedSize =
        aligned(aligned(sizeof(vertexData), uniAlign) * concurrentFrameCount + 1, uniAlign);
    ...
    // TODO: Seperate from here.
    quint8* data;
    result = devFuncs->vkMapMemory(device, m_bufMem, 0, memRequirements.size, 0,
    ...
    // declare number of attributes and layout for the vertex data for the
    // pipeline.
    ...
    VkDescriptorPoolSize descPoolSizes = {VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER,
                                          uint32_t(be->concurrentFrameCount())};
    ...
    result =
        devFuncs->vkCreateDescriptorSetLayout(device, &descLayoutInfo, nullptr, &m_descSetLayout);
    if (result != VK_SUCCESS)
    ...
    result =
        devFuncs->vkCreatePipelineLayout(device, &pipelineLayoutInfo, nullptr, &m_pipelineLayout);
    if (result != VK_SUCCESS)
    ...
    result = devFuncs->vkCreateGraphicsPipelines(device, m_pipelineCache, 1, &pipeInfo, nullptr,
                                                 &m_pipeline);
    if (result != VkResult::VK_SUCCESS) {
...
void vkRend::initSwapChainResources() {
    ...
    // this is the camera; it controls FOV, aspect ratio, clip spaces and
    // transforms.
...
void vkRend::startNextFrame() {
    ...
    devFuncs->vkCmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, m_pipelineLayout, 0, 1,
                                      &m_descSet[be->currentFrame()], 0, nullptr);
...
VkShaderModule vkRend::createShaderModule(const QString& name) {
    ...
    VkResult result =
        devFuncs->vkCreateShaderModule(be->device(), &shaderInfo, nullptr, &shaderModule);
    if (result != VK_SUCCESS) {
